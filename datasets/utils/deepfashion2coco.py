...
def parse_data(data_dir):
    num_images = len(os.listdir(data_dir + '/annos'))

    category_to_pairs_dict = {}
    ...
    for num in tqdm(range(1, num_images + 1)):
        json_name = data_dir + '/annos/' + str(num).zfill(6) + '.json'
        image_name = data_dir + '/image/' + str(num).zfill(6) + '.jpg'
                ...
                for i in temp:
                    if i == 'source' or i == 'pair_id':
                        ...
                        dataset['annotations'].append({
                            ...
                            'iscrowd': 0,
                            'style': style,
                            'num_keypoints': 0,#num_points,
                            'keypoints': [], #points.tolist(),
                            'segmentation': seg,
                        })

                        if not cat in category_to_pairs_dict:
                            category_to_pairs_dict[cat] = {}
                        if not pair_id in category_to_pairs_dict[cat]:
                            category_to_pairs_dict[cat][pair_id] = [[], []]

                        if source == 'shop':
                            category_to_pairs_dict[cat][pair_id][0].append({
                            'image_id': num,
                            'pair_id': pair_id,
                            'source': source,
                            'annot_id': sub_index
                            ...
                            category_to_pairs_dict[cat][pair_id][1].append({
                                'image_id': num,
                                'pair_id': pair_id,
                                'source': source,
                                'annot_id': sub_index
    ...
    dataset['category_to_pairs'] = category_to_pairs_dict

    gc.collect()

    return dataset

def save_dataset(path_dataset, split_type):
    json_name = '/home/firiuza/MachineLearning/DeepFashion2/splits_annotations/%s_deepfashion2.json' % split_type

    dataset = parse_data(path_dataset)
...
if __name__ == '__main__':
    ...
    for i_p, path in enumerate(['/home/firiuza/MachineLearning/DeepFashion2/train']):
                 # '/home/firiuza/MachineLearning/DeepFashion2/validation']):
        print(splits[i_p])
        save_dataset(path, splits[i_p])
